...
"""
# Adapted from Google's humanize module. Apache 2.0 License.
...
def PluralWord(quantity, singular, plural=None):
  ...
  # function simple!  It doesn't need to know about every possibility;
...
def DecimalScale(quantity, unit, min_scale=0, max_scale=None):
  """Get the scaled value and decimal prefixed unit in a tuple.
...
def Duration(duration, separator=" "):
  ...
  Args:
    duration: A duration, either as number of seconds, or a timedelta.
...
def NaturalSortKey(data):
  ...
  class _StrComparableInt(object):
    """The original implementation relies on Python 2's broken str/int
    comparison. This class provides equivalent behavior."""
